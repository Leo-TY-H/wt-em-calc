...
from propulsion_model import prepare,step
from propulsion_native import PropulsionNative

def main():
    m=PropulsionNative();rng=random.Random(300420);counts={};failures=[];hooks=set()
    ...
    for n in ['yak-3','bf-109f-4']:
        ...
        for i in range(1000):
            ...
            a=m.propulsion(p,s,**kw);e=step(p,s,**kw);check(n,i,a,e)
            if failures:break
        if failures:break
        ...
        for i in range(1200):
            ...
            a=m.propulsion(p,ns,seed=nseed,**kw);e=step(p,ps,seed=pseed,**kw);check(n+'_chained',i,a,e)
            if failures:break
    ...
    report=dict(status='FAIL' if failures else 'PASS',binary_sha256=m.sha,counts=counts,substituted_calls=sorted(hooks),failures=failures[:2],
                scope='Complete original101a10db0 return with original101a08de0 propeller,1019fa1b0 engine outer wrapper,1019f3b80 piston and1019f3290 running lifecycle. Prepared intact connected one-engine/one-propeller drivetrain. Thermal/fuel bookkeeping/damage bypassed for frozen fuel/health. All aggregate force, moment, angular momentum, wake, RPM, blade/governor/wake and engine fields compared; independent chained shaft, governor, regulator, modulation and random-stream histories. No whole-aircraft integration yet.')
    Path('analysis/propulsion-model-validation.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(report,indent=2))
