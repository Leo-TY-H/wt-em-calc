...
The two-OCPU deployment explicitly selects `WT_EM_WORKERS=2` and sets `OPENBLAS_NUM_THREADS`, `OMP_NUM_THREADS`, and `MKL_NUM_THREADS` to `1` before Python starts. Worker selection caps the explicit count at the CPU affinity and container quota; desktop defaults still reserve one CPU. The API still processes one chart at a time with its persistent numerical pool. These settings take effect on the next Oracle deployment.
